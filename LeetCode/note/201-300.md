...
struct ListNode* reverseList(struct ListNode* head){
    struct ListNode *new_head, *ptr, *tmp;
...
int countNodes(struct TreeNode* root){
    ...
    if(root) {
        node_count = 1;
        node_count += (countNodes(root->left) + countNodes(root->right));
    }
...
https://leetcode.cn/problems/count-univalue-subtrees/description/
...
void preOrderTraverse(struct TreeNode* root, int *count) {
    if(root) {
        if(isUnivalTree(root)) {
            *count += 1;
        }

        preOrderTraverse(root->left, count);
        preOrderTraverse(root->right, count);
    }
}
...
```c
void preOrderTraverse(struct TreeNode *root, double target, double *closest_val) {
    if(root) {
        if(fabs(root->val-target) < fabs(*closest_val-target)) {
            *closest_val = root->val;
        }
        preOrderTraverse(root->left, target, closest_val);
        preOrderTraverse(root->right, target, closest_val);
    }
}
...
int closestValue(struct TreeNode* root, double target){
    ...
    preOrderTraverse(root, target, &closest_val);
