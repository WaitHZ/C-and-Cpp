...
MinStack* minStackCreate() {
    ...
    obj->stack = makeNewStack(30000);

    return obj;
}

void minStackPush(MinStack* obj, int val) {
    if(isEmptyStack(obj->stack)) {
        obj->min = val;
    }
    else {
...
void minStackPop(MinStack* obj) {
    if(getTopEle(obj->stack) != obj->min) {
        pop(obj->stack);
    }
    else {
        pop(obj->stack);
        ...
        if(!isEmptyStack(obj->stack)) {
            obj->min = obj->stack->stackArr[0];

            for(int i = 1; i <= obj->stack->top; i++) {
                if(obj-> min > obj->stack->stackArr[i]) {
                    obj-> min = obj->stack->stackArr[i];
                }
            }
        }
    }
...
TwoSum* twoSumCreate() {
    ...
    obj = (TwoSum*)malloc(sizeof(TwoSum));
    obj->two_sum_arr = (int*)malloc(sizeof(int)*10000);
    obj->set = makeNewHashSet(10000);
    obj->index = 0;
...
void twoSumAdd(TwoSum* obj, int number) {
    (obj->two_sum_arr)[(obj->index)++] = number;
    HashSetAdd(obj->set, number);
}
...
bool twoSumFind(TwoSum* obj, int value) {
    ...
    for(int i = 0; i < obj->index; i++) {
        if(value % 2 == 0 && value / 2 == (obj->two_sum_arr)[i]) {
            for(int j = i+1; j < obj->index; j++) {
                if((obj->two_sum_arr)[i] == (obj->two_sum_arr)[j]) {
                    is_find = true;
                    break;
                }
            }
            ...
            if(HashSetContains(obj->set, value-(obj->two_sum_arr)[i])) {
                is_find = true;
                break;;
            }
        }
...
void rotate(int* nums, int numsSize, int k){
    ...
    for(int i = 0; i < numsSize; i++) {
        tmp[i] = nums[(i+numsSize-k)%numsSize];
    }
